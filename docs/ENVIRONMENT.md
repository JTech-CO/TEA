...
| 턴 상한 | `--max-turns 15` — 폭주 방지용. 절단은 오류가 아니라 별도 상태로 분류 |
...
| 트라이얼 분류 | `fired` 발화(절단 여부 무관 — 유효) / `quiet` 자연종료·미발화(유효) / `ambiguous` 절단·미발화(**판정 보류**) / `error` 실행 실패. 보류·실패는 게이트를 막는다 |
| 반복·집계 | 문항당 3회. 결과는 `tea/evals/results/`에 저장(문항×회차 매트릭스, 최초 발화 턴, 절단·거부 카운트 포함) |
